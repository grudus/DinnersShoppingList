...
spec =
    describe "Finding total price of ingredients" $ do
        it "Should return 0 when pricing or ingredient list is empty" $ do
            calculateTotalPrice [] [] `shouldBe` 0
            calculateTotalPrice pricing [] `shouldBe` 0
            calculateTotalPrice [] [Ingredient "a" 1.0 Kg] `shouldBe` 0
        
        it "Should return 0 when no ingredients in pricing" $
            calculateTotalPrice pricing [Ingredient "xx" 1.0 Item] `shouldBe` 0
        
        it "Should return 0 when name matches, but unit doesn't" $
            calculateTotalPrice pricing [Ingredient "a" 1.0 Pack] `shouldBe` 0
        
        it "Should calculate correctly" $
            calculateTotalPrice pricing [Ingredient "a" 1.0 Kg, Ingredient "b" 1.0 Pack] `shouldBe` 8.0

        it "Should calculate correctly when amount /= 1" $
            calculateTotalPrice pricing [Ingredient "a" 0.1 Kg, Ingredient "b" 10.0 Pack] `shouldBe` 30.5

        it "Should calculate correctly when multiple identical ingredients" $
            calculateTotalPrice pricing [Ingredient "c" 1 Kg, Ingredient "c" 2 Kg] `shouldBe` 31.5

        it "Should calculate correctly when multiple identical ingredients and no pricing available for some ingredients" $
            calculateTotalPrice pricing ingredients `shouldBe` 14.0
                where
                    ingredients = [
                        Ingredient "a" 0.2 Kg, 
